...
We import the data set "pols_month.csv". Since we have "2"s in column prez_gop, and, after checking with WIKI, the U.S president for that period is Ford, who is a republican, we replaced the "2" to "1" (1 for yes). We have variables containing the president, number of governors, senators and representatives in both parties, year and month. It's dimension is `r nrow(pols_month)`X`r ncol(pols_month)`; year range is `r range(pols_month$year)`; the key variables are year and month, meaning that we can identify any observations with a combined year and month.  
